...
const formatKind = type => {
  const singularType = singular(type);
  return _.capitalize(singularType);
};
...
module.exports = function createQueryBuilder({ model, modelKey, strapi }) {
  ...
  async function findOne(params, populate, { transacting } = {}) {
    ...
    if (entry && entry.serialize) {
      try {
        entry._serialize = entry.serialize;
        entry.serialize = function(options) {
          const _self = this;
          ...
          return entry._serialize(_self, options);
        };
      } catch (e) {
        console.error(e);
      }
