...
module.exports = function createQueryBuilder({ model, modelKey, strapi }) {
  ...
  async function findOne(params, populate, { transacting } = {}) {
    ...
    // YC Modified - support polymorphic graphql serialisation
    const forged = model.forge(params);
    const entry = await forged.fetch({
      withRelated: populate,
      transacting
    });

    forged._serialize = forged.serialize;
    forged.serialize = function(options) {
      const _self = this;
      const relationKeys = Object.keys(_self.relations);
      relationKeys.forEach(relationKey => {
        const relation = _self.relations[relationKey];
        if (relation.models && Array.isArray(relation.models)) {
          relation.models.forEach(model => {
            const type = model.attributes[`${relationKey}_type`];
            if (type) {
              const kind = _.capitalize(pluralize.singular(type));
              model.relations.related.attributes.kind = kind;
            }
          });
        }
      });

      return forged._serialize(_self, options);
    };
